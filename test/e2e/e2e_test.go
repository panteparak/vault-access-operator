...
var _ = Describe("Manager", Ordered, func() {
	...
	BeforeAll(func() {
		...
		utils.TimedBy("checking if operator is already deployed")
		...
		utils.TimedBy("labeling the namespace to enforce the restricted security policy")
	...
	AfterAll(func() {
		utils.TimedBy("cleaning up the curl pod for metrics")
		cmd := exec.Command("kubectl", "delete", "pod", "curl-metrics", "-n", namespace, "--ignore-not-found")
		_, _ = utils.Run(cmd)

		// Skip cleanup if operator was pre-deployed by CI (other tests depend on it)
		if operatorAlreadyDeployed {
			utils.TimedBy("skipping cleanup - operator was pre-deployed by CI")
			return
		}

		utils.TimedBy("undeploying the controller-manager")
	...
	AfterEach(func() {
		...
		if specReport.Failed() {
			utils.TimedBy("Fetching controller manager pod logs")
			...
			utils.TimedBy("Fetching Kubernetes events")
			...
			utils.TimedBy("Fetching curl-metrics logs")
			...
			utils.TimedBy("Fetching controller manager pod description")
	...
	Context("Manager", func() {
		It("should run successfully", func() {
			utils.TimedBy("validating that the controller-manager pod is running as expected")
		...
		It("should ensure the metrics endpoint is serving metrics", func() {
			utils.TimedBy("creating a ClusterRoleBinding for the service account to allow access to metrics")
			cmd := exec.Command("kubectl", "create", "clusterrolebinding", metricsRoleBindingName,
				"--clusterrole=vault-access-operator-metrics-reader",
				fmt.Sprintf("--serviceaccount=%s:%s", namespace, serviceAccountName),
			)
			_, err := utils.Run(cmd)
			Expect(err).NotTo(HaveOccurred(), "Failed to create ClusterRoleBinding")

			utils.TimedBy("validating that the metrics service is available")
			cmd = exec.Command("kubectl", "get", "service", metricsServiceName, "-n", namespace)
			_, err = utils.Run(cmd)
			Expect(err).NotTo(HaveOccurred(), "Metrics service should exist")

			utils.TimedBy("getting the service account token")
			token, err := serviceAccountToken()
			Expect(err).NotTo(HaveOccurred())
			Expect(token).NotTo(BeEmpty())

			utils.TimedBy("waiting for the metrics endpoint to be ready")
			...
			utils.TimedBy("verifying that the controller manager is serving the metrics server")
			...
			utils.TimedBy("creating the curl-metrics pod to access the metrics endpoint")
			...
			utils.TimedBy("waiting for the curl-metrics pod to complete.")
			...
			utils.TimedBy("getting the metrics by checking curl-metrics logs")
