...
func TestDriftSuite(t *testing.T) {
	RegisterFailHandler(Fail)

	suiteConfig, reporterConfig := GinkgoConfiguration()
	reporterConfig.Verbose = true

	RunSpecs(t, "Drift Integration Test Suite", suiteConfig, reporterConfig)
}
...
var _ = BeforeSuite(func() {
	...
	By("Setting up drift test context")
	ctx, cancel := context.WithCancel(context.Background())
	integration.SetContext(ctx, cancel)

	By("Creating drift test environment")
	...
	By("Starting drift test environment")
	...
	DeferCleanup(func() {
		By("Stopping drift test environment")
