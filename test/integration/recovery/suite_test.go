...
func TestRecoverySuite(t *testing.T) {
	...
	RunSpecs(t, "Recovery Integration Test Suite", suiteConfig, reporterConfig)
}
...
var _ = BeforeSuite(func() {
	...
	By("Setting up recovery test context")
	ctx, cancel := context.WithCancel(context.Background())
	integration.SetContext(ctx, cancel)

	By("Creating recovery test environment")
	...
	By("Starting recovery test environment")
	...
	DeferCleanup(func() {
		By("Stopping recovery test environment")
