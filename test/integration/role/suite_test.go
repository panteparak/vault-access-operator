...
func TestRoleSuite(t *testing.T) {
	...
	RunSpecs(t, "Role Integration Test Suite", suiteConfig, reporterConfig)
}
...
var _ = BeforeSuite(func() {
	...
	By("Setting up role test context")
	ctx, cancel := context.WithCancel(context.Background())
	integration.SetContext(ctx, cancel)

	By("Creating role test environment")
	...
	By("Starting role test environment")
	...
	DeferCleanup(func() {
		By("Stopping role test environment")
