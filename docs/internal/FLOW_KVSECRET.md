...
`Phase=Active`, `Managed=true`, and a `Binding` is set (`vaultPath=spec.path`, `vaultResourceName=<rel>`, `boundAt`/`lastVerifiedAt=now`, `bindingVerified=true`). The base then calls `updateStatus`, which sets `Ready=True` / `Synced=True` and `lastSyncedAt`, and persists status. The operator does NOT publish an event (no event bus integration for this feature). Ownership lives in the seeded secret's own `custom_metadata` — the same in-band model policies use via their comment header (ADR 0008) — enriched with `auth-mount`, `cluster`, `managed-at`, and `last-updated`.
...
| 5 | Managed marker | In-band comment header inside the policy document (ADR 0008) | Native KV v2 `custom_metadata` on the seeded secret itself |
