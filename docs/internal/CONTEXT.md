...
- **Roles** — `alias_metadata` on the role object (same key vocabulary as the policy header; kubernetes + jwt backends, Vault ≥ 1.21 (older Vaults silently drop it) — [ADR 0010](../adr/0010-structured-vault-names-and-recorded-name-authority.md)). Backstopped by the owning CR's status plus the [Operator identity](#operator-identity) mount invariant — which is **true by construction** for roles: they carry no mount fields, so they can only ever land on their connection's [Role mount](#role-mount) ([ADR 0009](../adr/0009-connection-owned-role-mount.md)).
...
Every derived Vault resource name has the fixed 4-segment shape `vao.{identity}.{namespace}.{name}` (ADR 0010; `_` fills an absent segment). The identity segment is `--cluster-name` (/ `CLUSTER_NAME` / `clusterName` Helm value) when set, **else the connection's login auth mount** — already unique per cluster — else `_`. Lets multiple operators share one Vault CE server (no namespaces → a single global ACL policy store) without colliding on policy/role names. Orthogonal to the [Operator identity](#operator-identity): the name *prevents* collisions, the identity *detects and blocks* fights when names do collide. The RECORDED `status.vaultName`/`status.vaultRoleName` is authoritative for deletion and rename detection — a naming config change migrates objects (write new, delete old) instead of orphaning them. See [`shared/naming/`](../../shared/naming/naming.go), [ADR 0010](../adr/0010-structured-vault-names-and-recorded-name-authority.md), [`configuration.md`](../configuration.md#sharing-one-vault-across-clusters).
