...
| `vault.platform.io/restore-managed-markers=true` *(VaultConnection only)* | one-shot mass re-adoption: the connection reconciler walks every dependent VaultPolicy/VaultClusterPolicy/VaultRole/VaultClusterRole and re-writes its managed-marker entry in Vault. Use after the `secret/data/vault-access-operator/managed/` KV tree is wiped (manual cleanup, snapshot restore, etc.) so dependents aren't conflict-blocked. Auto-clears on success; partial failures keep it set so the next reconcile retries. Constant `AnnotationRestoreManagedMarkers` ([§G](IMPROVEMENTS.md#g-no-backuprestore-story-for-managed-markers-still-missing)). |
