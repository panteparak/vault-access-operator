...
// TestVaultPolicyValidator_NoCollisionChecks pins ADR 0010: CR pairs that
// collided under the "{namespace}-{name}" dash join are now structurally
// distinct (vao.{id}.{ns}.{name} vs vao.{id}._.{name}) and MUST be admitted.
func TestVaultPolicyValidator_NoCollisionChecks(t *testing.T) {
	// VaultClusterPolicy "test-ns-my-policy" — the exact string that used to
	// collide with VaultPolicy test-ns/my-policy.
	existing := &vaultv1alpha1.VaultClusterPolicy{
		ObjectMeta: metav1.ObjectMeta{Name: "test-ns-my-policy"},
		Spec: vaultv1alpha1.VaultClusterPolicySpec{
			ConnectionRef: "conn",
			Rules:         []vaultv1alpha1.PolicyRule{{Path: "secret/*", Capabilities: []vaultv1alpha1.Capability{"read"}}},
		},
	}
	scheme := runtime.NewScheme()
	_ = vaultv1alpha1.AddToScheme(scheme)
	v := &VaultPolicyValidator{client: fake.NewClientBuilder().WithScheme(scheme).WithObjects(existing).Build()}
	policy := &vaultv1alpha1.VaultPolicy{
		ObjectMeta: metav1.ObjectMeta{Name: "my-policy", Namespace: "test-ns"},
		Spec: vaultv1alpha1.VaultPolicySpec{
			ConnectionRef: "conn",
			Rules:         []vaultv1alpha1.PolicyRule{{Path: "secret/*", Capabilities: []vaultv1alpha1.Capability{"read"}}},
		},
	...
	if _, err := v.ValidateCreate(context.Background(), policy); err != nil {
		t.Errorf("ADR 0010: formerly-colliding pair must be admitted, got %v", err)
	}
}
